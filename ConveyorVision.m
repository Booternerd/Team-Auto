...
%     close all; clear;
%     image_file_path='C:\Users\jeffry\Documents\UNSW\SEMESTER_2_2016\MTRN4230\MATLAB\Assignment1\Training_Images\gte5.png';
...
function detectJenga(BW,RGB)
    ...
    eroded = imorph(clean, kcircle(2), 'min');
    figure; idisp(eroded); 
    ...
    if (isempty(ib)==1)  
        ...
        for ii=1:length(ib.uc)
            if(ib(ii).area_<800)
                ...
                output.b = [output.b inpolygon(uc,vc,box.area(:,1),box.area(:,2))];
%                 output.theta=[output.theta theta];
            elseif(ib(ii).area_>1800 && (ib(ii).b/ib(ii).a)>0.4)
                ...
                crop=iroi(RGB,[ib(ii).umin-10 ib(ii).umax+10; ib(ii).vmin-10, ib(ii).vmax+10]);
                size=side_analysis(crop,ib(ii).area_,uc,vc,ib(ii).umin-10,ib(ii).vmin-10);
                if(size==1)
                    label=1;
...
function convert_pixels_to_mm()
    ...
    output.y = (xt - xt2).*xp2mmC;
    output.x = (yt - yt2).*yp2mmC;
    ...
    box.b1(2) = (xt - xt2).*xp2mmC;
    box.b1(1) = (yt - yt2).*yp2mmC;
    %% b2
    ...
    box.b2(2) = (xt - xt2).*xp2mmC;
    box.b2(1) = (yt - yt2).*yp2mmC;
    %% b3
    ...
    box.b3(2) = (xt - xt2).*xp2mmC;
    box.b3(1) = (yt - yt2).*yp2mmC;
    %% b4
    ...
    box.b4(2) = (xt - xt2).*xp2mmC;
    box.b4(1) = (yt - yt2).*yp2mmC;
end
